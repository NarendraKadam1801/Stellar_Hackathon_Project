...
class TestScenarios {
  ...
  async runAuthenticationTests() {
    ...
    try {
      ...
      // NOTE: The previous run failed here because the server responded with 200 instead of 401. 
      // The test is written to expect 401.
      ...
      // Re-throw if it's a test assertion failure, but use a specific message
      if (error.message.includes('Expected error status 401, got 200')) {
         ...
         // Throw a specific error to be caught by the scenario runner
  ...
  async runPostTests(accessToken) {
    ...
    if (!this.testUser) {
      // This should not happen if auth ran first, but guards against running posts alone.
  ...
  // You would add runPaymentTests, runIpfsTests, runExpensesTests here
}
