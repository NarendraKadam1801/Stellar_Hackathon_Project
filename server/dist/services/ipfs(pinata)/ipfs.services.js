...
const retriveFromIpfs = async (cid) => {
    try {
        ...
        if (!data)
            throw new Error("Somethign went wrong while retriving data from ipfs");
        return {
            success: true,
            data,
            cid
...
const uploadOnIpfs = async (data) => {
    try {
        ...
        if (!uploadData)
            throw new Error("Somethign went wrong while uploading data from ipfs");
        return {
            success: true,
            cid: uploadData.cid,
        };
...
const deleteIpfsData = async (cid) => {
    try {
        ...
        if (!dataDelet)
            throw new Error("Somethign went wrong while deleting data from ipfs");
        return {
            success: true,
            cid,
        };
