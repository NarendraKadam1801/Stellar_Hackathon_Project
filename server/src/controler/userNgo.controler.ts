...
const normalizeAndValidateUserData = (data: any): userSingupData => {
  ...
  return normalizedData as userSingupData;
};
...
const singup = AsyncHandler(async (req: Request, res: Response) => {
  ...
  if (userData.email) {
    ...
    if(findUserData && findUserData.length > 0) throw new ApiError(401,"User already exists");
  }
