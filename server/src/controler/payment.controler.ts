...
const walletPay = AsyncHandler(async (req: Request, res: Response) => {
  ...
  if (!senderWallet) throw new ApiError(400, "Wallet data is required");
  if (!senderWallet.PostId) throw new ApiError(400, "Post ID is required");
  if (!senderWallet.PublicKey) throw new ApiError(400, "Recipient wallet address is required");
